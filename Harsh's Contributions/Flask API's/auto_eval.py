...
def get_eval(actual, query):
    ...
    if int(perc_scores)==100:
        marks_deduct=0
    elif (int(perc_scores))>90 and (int(perc_scores))<100:
        marks_deduct=0.01
    elif(int(perc_scores))>80 and (int(perc_scores))<90:
        marks_deduct=0.05*total
    elif (int(perc_scores)) > 70 and (int(perc_scores)) < 80:
        marks_deduct = 0.15 * total
    elif (int(perc_scores)) > 60 and (int(perc_scores)) < 70:
        marks_deduct = 0.25 * total
    elif (int(perc_scores)) > 50 and (int(perc_scores)) < 60:
        marks_deduct = 0.35 * total

    tot_mks=total-marks_deduct
    res = " Corupus Similarity Ratio:" + str(round(perc_scores, 2)) + "%"+"\n"+"Result as Marks: "+str(tot_mks)
